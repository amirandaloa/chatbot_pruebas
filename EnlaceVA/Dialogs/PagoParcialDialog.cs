...
{
    public class PagoParcialDialog : ComponentDialog
    {
        ...
        public PagoParcialDialog(
            IServiceProvider serviceProvider,
            IBotTelemetryClient telemetryClient)
            : base(nameof(PagoParcialDialog))
        {
            ...
            var PagoAnticipadoSteps = new WaterfallStep[]
            {
                GetContractAsync,
                CheckContract,
                GetTypeIdAsync,
            ...
            AddDialog(new WaterfallDialog(nameof(PagoParcialDialog), PagoAnticipadoSteps));
        }
        ...
        private async Task<DialogTurnResult> GetContractAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            userProfile.Dialogturn = "PagoParcialDialog";
        ...
        private async Task<DialogTurnResult> CheckContract(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                return await stepContext.ReplaceDialogAsync("PagoParcialDialog");
            }
        }
        ...
        private async Task<DialogTurnResult> GetPayAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            await _userState.SaveChangesAsync(stepContext.Context, true);
            ...
            DatosEntradaSaldoPendiente saldoPendienteEntrada;


            saldoPendienteEntrada = new DatosEntradaSaldoPendiente()
            {
                contrato = information.contrato,
            };

            var result = _transactionalServices.GetSaldoPendiente(information.company, saldoPendienteEntrada);

            if (result.mensajes.CodMsj == "0")
            {
                await stepContext.Context.SendActivityAsync(_templateEngine.GenerateActivityForLocale("SaldoPendienteResult", result), cancellationToken: cancellationToken);
                information.saldoPendiente = result.saldoPendiente;
            }
            else if (result.mensajes.CodMsj == "0")
            {
                await stepContext.Context.SendActivityAsync(_templateEngine.GenerateActivityForLocale("SinSaldoPendiente", result), cancellationToken: cancellationToken);
            }
            else
            {
            ...
            await stepContext.Context.SendActivityAsync(_templateManager.GenerateActivityForLocale("GetPay"), cancellationToken: cancellationToken);
        ...
        protected override async Task<DialogTurnResult> OnContinueDialogAsync (DialogContext innerDc, CancellationToken cancellationToken = default)
        {
            ...
            if (userProfile.Dialogturn == "PagoParcialDialog")
            {
                ...
                switch (validation)
                {
                    ...
                    case MessagesCertificadoFacturaDialog.TextPhone:
                        ...
                        return await innerDc.ContinueDialogAsync();
                    case MessagesCertificadoFacturaDialog.TextPayError:
                        GetInformation.pago = null;
                        await innerDc.Context.SendActivityAsync(_templateManager.GenerateActivityForLocale("PagoMayorResult"), cancellationToken: cancellationToken);
                        return new DialogTurnResult(DialogTurnStatus.Waiting);
                    case MessagesCertificadoFacturaDialog.TextPay:
                        GetInformation.pago = Formatmoney(text,userProfile.Company);
                        GetInformation.pagoParcial = text;
                        await innerDc.Context.SendActivityAsync(_templateManager.GenerateActivityForLocale("ValidarPago", GetInformation), cancellationToken: cancellationToken);
                        return new DialogTurnResult(DialogTurnStatus.Waiting);
                    case MessagesCertificadoFacturaDialog.TextConfirmPay:
                        GetInformation.confirmarPago = text;

                        if (GetInformation.confirmarPago == "1")
                        {
                            return await innerDc.ContinueDialogAsync();
                        }
                        else
                        {
                            GetInformation.pago = null;
                            GetInformation.confirmarPago = null;
                            await innerDc.Context.SendActivityAsync(_templateManager.GenerateActivityForLocale("GetNewPay"), cancellationToken: cancellationToken);
                            return new DialogTurnResult(DialogTurnStatus.Waiting);
                        }
                    case MessagesCertificadoFacturaDialog.TextPagoErroneo:
                        await innerDc.Context.SendActivityAsync("PagoMayorResult");
                        return new DialogTurnResult(DialogTurnStatus.Waiting);
        ...
        {            
            ...
            else if (information.pago == null)
            {
                message = ValidatePay(data, information.saldoPendiente);
            }
            else if (information.confirmarPago == null)
            {
                message = ValidateConfirmPay(data);
            }
            else
            {
                message = "Error";
