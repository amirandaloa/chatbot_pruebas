...
{
    public class ConceptoFacturaDialogCard : ComponentDialog
    {        
        ...
        public ConceptoFacturaDialogCard(
            IServiceProvider serviceProvider,
            IBotTelemetryClient telemetryClient)
            : base(nameof(ConceptoFacturaDialogCard))
        {
            ...
            AddDialog(new WaterfallDialog(nameof(ConceptoFacturaDialogCard), CertificadoFacturaSteps));
        }        
        ...
        private async Task<DialogTurnResult> DataTreatment(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                return await stepContext.ReplaceDialogAsync("ConceptoFacturaDialogCard");
            }
        }
        ...
        private async Task<DialogTurnResult> SendData(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var result = _transactionalServices.GetConceptoFactura(company, conceptoFacturaEntrada);
            result.contrato = information.contrato;
            if (result.mensajes.CodMsj == "0")
            {
                await stepContext.Context.SendActivityAsync(_templateEngine.GenerateActivityForLocale("ConceptoFacturaResult", result), cancellationToken: cancellationToken);
            }
            else
            {
        ...
        protected override async Task<DialogTurnResult> OnContinueDialogAsync(DialogContext innerDc, CancellationToken cancellationToken = default)
        {
            ...
            if (userProfile.Dialogturn == "ConceptoFacturaDialogCard")
            {
